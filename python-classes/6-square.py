...
class Square:
    ...
    def __init__(self, size=0, position=(0, 0)):
        ...
        if not isinstance(size, int):
            ...
            self.__size = size
        if not isinstance(position, tuple) or position[0] < 0 or position[1] < 0 or position[1] == None:
    ...
    def position(self, value):
        if not isinstance(value, tuple) or value[0] < 0 or value[1] < 0 or value[1] == None:
            raise TypeError("position must be a tuple of 2 positive integers")
